...
postForm.addEventListener('submit', async (e) => {
    ...
    const postData = {
        title,
        date,
        thumbnail: 'none', // No longer using thumbnails
        type: postType,
        external_link: postType === 'external' ? normalizeUrl(externalLink) : null,
        content: postType === 'content' ? content : null,
        user_id: currentUser.id
    };nst { data: post, error } = await supabase
        .from('posts')
        .select('*')
        .eq('id', postId)
        .single();
...
supabase.auth.onAuthStateChange((event, session) => {
    ...
    postsContainer.innerHTML = posts.map(post => {
        ...
        const linkIndicator = post.type === 'external' ? '<div class="post-link-indicator">🔗 External link</div>' : '';
        const contentDisplay = post.type === 'content' ? `<div class="post-content">${post.content.substring(0, 100)}...</div>` : '';
